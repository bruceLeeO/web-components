(function(window) {
  ...
  var next = document.getElementsByClassName("accordion-foursided-carousel-next");
  var prev = document.getElementsByClassName("accordion-foursided-carousel-prev");

  var dots = document.getElementsByClassName("afca-dot");


  var dot1 = document.getElementById("afca-dot1");
  var dot2 = document.getElementById("afca-dot2");
  var dot3 = document.getElementById("afca-dot3");
  var dot4 = document.getElementById("afca-dot4");
  ...
  dot1.onclick = function(evt) {
    ...
    for (var i = 0; i < dots.length; i++) {
      ...
      if (isExpressionInArrs(dots[i].className,"dot--active")) {
        if (dots[i].id === "afca-dot2") {
          currdeg += 90;
        }
        if (dots[i].id === "afca-dot3") {
          currdeg += 180;
        }   
        if (dots[i].id === "afca-dot4") {
          currdeg += 270;
        }     
  ...
  next[0].onclick = function(evt) {
    ...
    for (var i = 0; i < dots.length; i++) {
      ...
      if (isExpressionInArrs(dots[i].className,"dot--active")) {
        ...
        dots[i].className =dots[i].className.replace(" dot--active","");
        //console.log("replace active in dots["+i+"].className "+dots[i].className);
        if (dots[i].id === "afca-dot1" || dots[i].id === "afca-dot2" || dots[i].id === "afca-dot3") {
  ...
  prev[0].onclick = function(evt) {
    ...
    for (var i = 0; i < dots.length; i++) {
      ...
      if (isExpressionInArrs(dots[i].className,"dot--active")) {
        ...
        dots[i].className =dots[i].className.replace(" dot--active","");
        //console.log("replace active in dots["+i+"].className "+dots[i].className);
        if (dots[i].id === "afca-dot4" || dots[i].id === "afca-dot3" || dots[i].id === "afca-dot2") {
          dots[i-1].className += " dot--active";
          break;
        } else {
          if (dots[i].id === "afca-dot1") {
